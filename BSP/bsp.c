...
#ifdef STM32F10X_CL
/*********************Sysclk Config for STM32 connectivity line devices**********************/
	RCC_PREDIV2Config(RCC_PREDIV2_Div3); /* Fprediv2 = HSE      /  3 =  4MHz.                    */
	RCC_PLL2Config(RCC_PLL2Mul_10); /* PLL2     = Fprediv2 *  10 = 40MHz.                    */
	RCC_PLL2Cmd(ENABLE);
	RCC_PLL3Config(RCC_PLL3Mul_12); /* PLL3     = Fprediv2 * 12 = 48MHz.                    */
	RCC_PLL3Cmd(ENABLE);
...
void  BSP_LED_Toggle (CPU_INT08U led)
{
	...
	case 0:
		...
		GPIO_SetBits( GPIOE, pins & (GPIO_Pin_0 | GPIO_Pin_1));
		GPIO_ResetBits(GPIOE, (~pins) & (GPIO_Pin_0 | GPIO_Pin_1));

		pins = GPIO_ReadOutputData(GPIOB);
		pins ^= GPIO_Pin_8 | GPIO_Pin_9;
		GPIO_SetBits( GPIOB, pins & (GPIO_Pin_8 | GPIO_Pin_9));
		GPIO_ResetBits(GPIOB, (~pins) & (GPIO_Pin_8 | GPIO_Pin_9));
		break;

	case 1:
		pins = GPIO_ReadOutputDataBit(GPIOE, GPIO_Pin_0);
		if (pins == Bit_RESET)
			GPIO_SetBits(GPIOE, GPIO_Pin_0);
		else
			GPIO_ResetBits(GPIOE, GPIO_Pin_0);
	...
	case 2:
		pins = GPIO_ReadOutputDataBit(GPIOE, GPIO_Pin_1);
		if (pins == Bit_RESET)
			GPIO_SetBits(GPIOE, GPIO_Pin_1);
		else
			GPIO_ResetBits(GPIOE, GPIO_Pin_1);
	...
	case 3:
		pins = GPIO_ReadOutputDataBit(GPIOB, GPIO_Pin_8);
		if (pins == Bit_RESET)
			GPIO_SetBits(GPIOB, GPIO_Pin_8);
		else
			GPIO_ResetBits(GPIOB, GPIO_Pin_8);
	...
	case 4:
		pins = GPIO_ReadOutputDataBit(GPIOB, GPIO_Pin_9);
		if (pins == Bit_RESET)
			GPIO_SetBits(GPIOB, GPIO_Pin_9);
		else
			GPIO_ResetBits(GPIOB, GPIO_Pin_9);
